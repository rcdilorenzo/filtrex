defmodule FiltrexConditionDateTest do
    ...
    assert Date.parse(@options_config, %{
             ...
             {:ok,
              %Filtrex.Condition.Date{
                ...
                inverse: false,
                type: :date,
                value: Timex.to_date({2016, 12, 29})
              }}
  end
  ...
  test "encoding as SQL fragments for ecto" do
    assert encode(Date, @column, @default, "after") ==
             {"? > ?", [column_ref(:date_column), @default]}

    assert encode(Date, @column, @default, "on or after") ==
             {"? >= ?", [column_ref(:date_column), @default]}

    assert encode(Date, @column, @default, "before") ==
             {"? < ?", [column_ref(:date_column), @default]}

    assert encode(Date, @column, @default, "on or before") ==
             {"? <= ?", [column_ref(:date_column), @default]}

    assert encode(Date, @column, %{start: @default, end: "2015-12-31"}, "between") ==
             {"(? >= ?) AND (? <= ?)",
              [column_ref(:date_column), @default, column_ref(:date_column), "2015-12-31"]}

    assert encode(Date, @column, %{start: @default, end: "2015-12-31"}, "not between") ==
             {"(? > ?) AND (? < ?)",
              [column_ref(:date_column), "2015-12-31", column_ref(:date_column), @default]}
