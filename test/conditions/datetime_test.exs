defmodule FiltrexConditionDateTimeTest do
    ...
    assert DateTime.parse(@options_config, %{
             ...
             {:ok,
              %Filtrex.Condition.DateTime{
                ...
                inverse: false,
                type: :datetime,
                value: Timex.to_datetime({{2016, 4, 18}, {13, 30, 45}}, "GMT")
              }}
  end
  ...
  test "encoding as SQL fragments for ecto" do
    assert encode(DateTime, @column, @default, "after") ==
             {"? > ?", [column_ref(:datetime_column), @default_converted]}

    assert encode(DateTime, @column, @default, "on or after") ==
             {"? >= ?", [column_ref(:datetime_column), @default_converted]}

    assert encode(DateTime, @column, @default, "before") ==
             {"? < ?", [column_ref(:datetime_column), @default_converted]}

    assert encode(DateTime, @column, @default, "on or before") ==
             {"? <= ?", [column_ref(:datetime_column), @default_converted]}

    assert encode(DateTime, @column, @default, "equals") ==
             {"? = ?", [column_ref(:datetime_column), @default_converted]}

    assert encode(DateTime, @column, @default, "does not equal") ==
             {"? != ?", [column_ref(:datetime_column), @default_converted]}
  end
